...
def latinize(text: str, lookup_table: dict, precision: int = 3) -> str:
    lookups = str.maketrans(lookup_table)
    ...
    text = re.sub(r'(?<=[бвгджзклмнпрстфхцчшБВГДЖЗКЛМНПРСТФХЦЧШ])([Іі])', r'¤\1', text)
    ...
    text = re.sub(r'(?<=[бвгджзклмнпрстфхцчшБВГДЖЗКЛМНПРСТФХЦЧШÐðйЙ])¤([Іі])', r'\1', text)
